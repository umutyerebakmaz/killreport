...
async function populate() {
  ...
  const count = await prismaWorker.$queryRaw<
    Array<{ count: bigint }>
  >`SELECT COUNT(*) as count FROM alliance_top_corporation_targets`;
  logger.info(
    `✅ alliance_top_corporation_targets: ${Number(count[0].count).toLocaleString()} records in ${(duration / 1000).toFixed(2)}s`,
  );
}
