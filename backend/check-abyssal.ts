...
async function test() {
  ...
  if (abyssalSystems.length > 0) {
    abyssalSystems.forEach((s) => {
      console.log(
        `  System ${s.id}: ${s.name}, sec=${s.security_status}, class=${s.security_class}`,
      );
    });
  } else {
  ...
  `;
  console.log(
    '\nAbyssal killmails count:',
    abyssalKillmails[0].count.toString(),
  );

  // Check for systems with specific security_class that might indicate Abyssal
  const securityClasses = await prisma.$queryRaw<
    Array<{ security_class: string | null; count: bigint }>
  >`
