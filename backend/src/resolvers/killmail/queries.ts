...
import * as MostValuableService from '@services/killmail/most-valuable.service';
...
export const killmailQueries: QueryResolvers = {
  ...
  killmails: async (_, args) => {
    ...
    if (hasKillmailFiltersCompatibleFilter) {
      ...
      if (killmailIds.length === 0) {
        ...
        const ttl =
          page <= 3
            ? CACHE_TTL.KILLMAIL_LIST_FIRST_PAGES
            : CACHE_TTL.KILLMAIL_LIST;
      ...
      if (
        args.filter?.minValue !== undefined &&
        args.filter?.minValue !== null
      ) {
      ...
      if (
        args.filter?.maxValue !== undefined &&
        args.filter?.maxValue !== null
      ) {
        countValueConditions.push(`total_value <= $${countParamIdx++}`);
        mainValueConditions.push(`total_value <= $${mainParamIdx++}`);
      ...
      const countWhereClause =
        countValueConditions.length > 0
          ? ` AND ${countValueConditions.join(' AND ')}`
          : '';
      const mainWhereClause =
        mainValueConditions.length > 0
          ? ` AND ${mainValueConditions.join(' AND ')}`
          : '';
      ...
      const countDateWhereClause =
        countDateConditions.length > 0
          ? ` AND ${countDateConditions.join(' AND ')}`
          : '';
      const mainDateWhereClause =
        mainDateConditions.length > 0
          ? ` AND ${mainDateConditions.join(' AND ')}`
          : '';
      ...
      const orderByClause =
        orderBy === 'timeAsc'
          ...
          : orderBy === 'timeDesc'
            ? 'killmail_time DESC'
            : orderBy === 'valueAsc'
              ? 'total_value ASC NULLS LAST'
              : 'total_value DESC NULLS LAST'; // valueDesc

      const killmails = await prisma.$queryRawUnsafe<
        Array<{
          killmail_id: number;
      ...
      const prismaOrderBy =
        orderBy === 'timeAsc'
          ? { killmail_time: 'asc' as const }
          : orderBy === 'timeDesc'
            ? { killmail_time: 'desc' as const }
            : orderBy === 'valueAsc'
              ? { total_value: 'asc' as const }
  ...
  killmailsDateCounts: async (_, args, context) => {
    ...
    const cacheDurationSeconds = Math.floor(
      CACHE_TTL.KILLMAIL_LIST_FIRST_PAGES / 1000,
    );
    await redis.setex(cacheKey, cacheDurationSeconds, JSON.stringify(result));
