...
export const stargateDestinationFields: StargateDestinationResolvers = {
  destinationSystemId: (parent) =>
    (parent as any).destination_system_id ?? null,
  destinationStargateId: (parent) =>
    (parent as any).destination_stargate_id ?? null,
...
export const stationFields: StationResolvers = {
  ...
  reprocessingEfficiency: (parent) =>
    (parent as any).reprocessing_efficiency ?? null,
  reprocessingStationsTake: (parent) =>
    (parent as any).reprocessing_stations_take ?? null,
  officeRentalCost: (parent) => (parent as any).office_rental_cost ?? null,
  maxDockableShipVolume: (parent) =>
    (parent as any).max_dockable_ship_volume ?? null,
  position: (parent) => toPosition(parent),
  solarSystem: async (parent, _, context) =>
    context.loaders.solarSystem.load((parent as any).solar_system_id),
};
