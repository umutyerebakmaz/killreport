...
export const solarSystemQueries: QueryResolvers = {
  solarSystem: async (_, { id }) => {
    ...
    if (useRawQuery) {
      ...
      const rawResults = await prisma.$queryRawUnsafe(query, ...whereParams);

      // Map raw database column names to Prisma field names
      systems = (rawResults as any[]).map((row: any) => ({
        id: row.system_id,
        name: row.name,
