...
export const workerResolvers: Resolvers = {
    Query: {
        ...
        } = await checkWorkerProcess(worker.names);
    return {
        ...
        workerStatusUpdates: {
            subscribe: async function* () {
                ...
                while (true) {
                    ...
                    const queues = awaits = QUEUE_WORKER_MAP[queue.name];
                    let workerRunning = false;
                    let workerPid: number | undefined;
                    let workerName: string | undefined;
                    ...
                    if (workerNames) {
                        ...
                        workerRunning = running;
                        workerPid = pid;
                        workerName = matchedName || workerNames[0]; // Use matched name or first as fallback pid } = await checkWorkerProcess(workerName);
                        workerRunning = running;
                        workerPid = pid;
                    }
...
return {
    name: matchedName || worker.names[0], // Use matched name or first as fallback= await Promise.all(
    STANDALONE_WORKERS.map(async (worker) => {
        const { running, pid } = await checkWorkerProcess(worker.name);
        return {
            name: worker.name,
