...
export const typeFields: TypeResolvers = {
    ...
    jitaPrice: async (parent, _, context) => {
        const prismaType = parent as any;

        // MarketService zaten DB cache kullanıyor (4 saat fresh)
        const price = await MarketService.getJitaPrice(prismaType.id);
        ...
        return {
            buy: price.buy,
            sell: price.sell,
            average: price.average || (price.buy + price.sell) / 2,
