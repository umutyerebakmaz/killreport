...
export function organizeFitting(
    ...
    // DEBUG: Log all flags in flagGroups to see implant flags
    console.log('🔍 DEBUG: All flags in flagGroups:', Array.from(flagGroups.keys()).sort((a, b) => a - b));

    implantFlags.forEach((flag) => {
        const items = flagGroups.get(flag);
        if (items && items.length > 0) {
            console.log(`💎 Found implant at flag ${flag}:`, items[0].item_type_id);
            implants.push(convertToFittingModule(items));
        }
    });

    console.log(`💎 Total implants extracted: ${implants.length}`);
