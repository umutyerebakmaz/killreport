/**
 * Real-time Daily Aggregates Update Service
 *
 * Updates daily_pilot_kills, daily_corporation_kills, and daily_alliance_kills
    ...
    await Promise.all([
      ...
      ...uniqueCharacterIds.map(characterId =>
        tx.$executeRaw`
                    ...
                    DO UPDATE SET kill_count = daily_pilot_kills.kill_count + 1
                `
      ),
      ...
      ...uniqueCorporationIds.map(corporationId =>
        tx.$executeRaw`
                    ...
                    DO UPDATE SET kill_count = daily_corporation_kills.kill_count + 1
                `
      ),
      ...
      ...uniqueAllianceIds.map(allianceId =>
        tx.$executeRaw`
                    ...
                    DO UPDATE SET kill_count = daily_alliance_kills.kill_count + 1
                `
      ),
    ]);
    ...
    for (const [key, count] of pilotCounts.entries()) {
      ...
      queries.push(
        tx.$executeRaw`
                    ...
                    ON CONFLICT (kill_date, character_id)
                    DO UPDATE SET kill_count = daily_pilot_kills.kill_count + ${count}
                `
      );
    }
    ...
    for (const [key, count] of corpCounts.entries()) {
      ...
      queries.push(
        tx.$executeRaw`
                    ...
                    ON CONFLICT (kill_date, corporation_id)
                    DO UPDATE SET kill_count = daily_corporation_kills.kill_count + ${count}
                `
      );
    }
    ...
    for (const [key, count] of allianceCounts.entries()) {
      ...
      queries.push(
        tx.$executeRaw`
                    ...
                    ON CONFLICT (kill_date, alliance_id)
                    DO UPDATE SET kill_count = daily_alliance_kills.kill_count + ${count}
                `
      );
    }
