...
const PAGE_DELAY = 10000; // 10 seconds between pages (zKillboard requirement - same endpoint)
...
export async function getCharacterKillmailsFromZKill(
    ...
    logger.info(`  🔍 ${prefix} Fetching from zKillboard (max ${maxPages} pages, ${limit} per page)...`);
    ...
    for (let currentPage = page; currentPage <= maxPages; currentPage++) {
        ...
        for (let attempt = 1; attempt <= RETRY_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt < RETRY_ATTEMPTS) {
                    logger.warn(`     ⚠️  ${prefix} Attempt ${attempt}/${RETRY_ATTEMPTS} failed, retrying in ${RETRY_DELAY}ms...`);
                    await sleep(RETRY_DELAY);
                } else {
                    logger.error(`     ❌ ${prefix} All ${RETRY_ATTEMPTS} attempts failed for page ${currentPage}`);
                }
            }
        }
        ...
        try {
            ...
            try {
                ...
                logger.warn(`     ⚠️  ${prefix} Failed to parse JSON on page ${currentPage}, stopping`);
                break;
            }

            // Validate killmails is an array
            if (!Array.isArray(killmails)) {
                logger.warn(`     ⚠️  ${prefix} Invalid response format on page ${currentPage}, stopping`);
                break;
            }

            logger.debug(`     📄 ${prefix} Page ${currentPage}: ${killmails.length} killmails`);
            ...
            if (killmails.length < limit) {
                logger.info(`     ✓ ${prefix} Last page (${killmails.length} < ${limit})`);
                break;
            }
            ...
            logger.error(`     ❌ ${prefix} Error fetching page ${currentPage}:`, error);
            break;
        }
    }

    logger.info(`     ✅ ${prefix} Total: ${allKillmails.length} killmails from zKillboard`);
    return allKillmails;
}
