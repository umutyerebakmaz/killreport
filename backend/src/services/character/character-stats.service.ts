...
/**
 * Get top 10 ship types used by a character (as attacker)
 * Uses killmail_filters table with GIN indexes on attacker_character_ids array
 */
export async function getTopShips(
  characterId: number,
  filter?: string | null,
): Promise<Array<{ killCount: number; shipType: any }>> {
  const cacheKey = getCacheKey(characterId, 'top_ships', filter);
 ...
 */
export async function invalidateCharacterStats(
  characterId: number,
): Promise<void> {
