...
async function processSystem(systemId: number): Promise<boolean> {
    try {
        ...
        console.log(`✅ Saved system ${systemId} - ${data.name}`);
        ...
        if (error.response?.status === 404) {
            console.log(`⚠️  System ${systemId} not found (404)`);
...
async function startWorker() {
    try {
        ...
        console.log('🚀 Solar System Worker Started');
        console.log('==========================');
        ...
        channel.consume(
            ...
            async (msg) => {
                ...
                if (isNaN(systemId)) {
                    console.error('❌ Invalid system ID:', msg.content.toString());
                    channel.ack(msg);
                    errorCount++;
                    return;
                }
