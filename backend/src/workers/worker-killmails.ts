import '../config';
import { enrichKillmail } from '../services/enrichment';
        ...
        for (const zkillPkg of zkillPackages) {
            try {
                ...
                try {
                    ...
                    // Killmail başarıyla kaydedildiyse, bilinmeyen verileri ESI'dan çek
                    try {
                        const enrichResult = await enrichKillmail(zkillPkg.killmail_id);

                        if (enrichResult.charactersAdded > 0 || enrichResult.corporationsAdded > 0 ||
                            enrichResult.alliancesAdded > 0 || enrichResult.typesAdded > 0) {
                            console.log(`     🔍 Enriched: ${enrichResult.charactersAdded} chars, ${enrichResult.corporationsAdded} corps, ${enrichResult.alliancesAdded} alliances, ${enrichResult.typesAdded} types`);
                        }

                        if (enrichResult.errors.length > 0) {
                            console.log(`     ⚠️  Enrichment warnings: ${enrichResult.errors.length}`);
                        }
                    } catch (enrichError) {
                        console.error(`     ⚠️  Enrichment failed for ${zkillPkg.killmail_id}:`, enrichError);
                        // Enrichment hatası killmail kaydını etkilememeli, devam et
                    }
