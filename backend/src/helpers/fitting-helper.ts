...
export function organizeFitting(
  ...
  if (hasSubsystems) {
    ...
    const subsystemCount = maxSubsystemFlag - InventoryFlag.SubSystem0 + 1;
  ...
  const fuelBay = fuelBayItems.map((item) => convertToFittingModule([item]));

  const oreHoldItems = [
    ...(flagGroups.get(InventoryFlag.SpecializedOreHold) || []),
    ...(flagGroups.get(InventoryFlag.SpecializedAsteroidHold) || []),
  ];
  const oreHold = oreHoldItems.map((item) => convertToFittingModule([item]));
  ...
  const otherSpecialHoldFlags = [
    InventoryFlag.RafflesHangar,
    InventoryFlag.SpecializedShipHold,
    InventoryFlag.SpecializedSmallShipHold,
    InventoryFlag.SpecializedMediumShipHold,
    InventoryFlag.SpecializedLargeShipHold,
    InventoryFlag.SpecializedIndustrialShipHold,
    InventoryFlag.SpecializedAmmoHold,
    InventoryFlag.SpecializedCommandCenterHold,
    InventoryFlag.SpecializedMaterialBay,
    InventoryFlag.QuafeBay,
    InventoryFlag.BoosterBay,
    InventoryFlag.SubsystemBay,
    InventoryFlag.FrigateEscapeBay,
    InventoryFlag.StructureDeedBay,
    InventoryFlag.CorpProjectsHangar,
    InventoryFlag.MoonMaterialBay,
  ];

  otherSpecialHoldFlags.forEach((flag) => {
    const items = flagGroups.get(flag);
    if (items) {
      otherSpecialHoldItems.push(...items);
    }
  });
