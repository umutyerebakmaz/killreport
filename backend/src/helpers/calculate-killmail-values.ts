...
export async function calculateKillmailValues(killmailData: {
    ...
    for (const item of items) {
        ...
        // Blueprint Copy (singleton !== 1) has no value, only Blueprint Original (singleton=1) has value
        // Note: singleton can be 0 (incorrectly stacked) or 2 (BPC) for copies
        const isBlueprint = blueprintMap.get(item.item_type_id) || false;
        const isBlueprintCopy = isBlueprint && item.singleton !== 1;

        if (isBlueprintCopy) {
            price = 0.01; // Blueprint copies have minimal value
        }
