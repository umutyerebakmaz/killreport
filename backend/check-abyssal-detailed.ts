...
async function test() {
  ...
  pochvenSystems.forEach((s) => {
    console.log(
      `  ${s.id}: ${s.name}, sec=${s.security_status}, class=${s.security_class}`,
    );
  });
  ...
  const highIdKillmails = await prisma.$queryRaw<
    Array<{ solar_system_id: number; count: bigint; name: string }>
  >`
  ...
  const securityStatusDistribution = await prisma.$queryRaw<
    Array<{ security_status: number | null; count: bigint }>
  >`
