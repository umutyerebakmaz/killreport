...
async function checkAttackerCounts() {
  ...
  samples.forEach((km) => {
    console.log(
      `  Killmail ${km.killmail_id}: attacker_count = ${km.attacker_count}, time = ${km.killmail_time.toISOString()}`,
    );
  });
  ...
  for (const km of randomSamples) {
    ...
    console.log(
      `  ${match} Killmail ${km.killmail_id}: cached=${cachedCount}, actual=${actualCount}`,
    );
  }
}
