...
function SovereigntyContent() {
  ...
  const maxRegionCount =
    hotRegions.reduce((m, r) => Math.max(m, r.campaignCount), 0) || 1;
    ...
    <>
      ...
      {overview && (
        <div className="grid grid-cols-2 gap-4 mt-6 md:grid-cols-3 lg:grid-cols-6">
          ...
          <StatCard
            label="Tracked structures"
            value={overview.trackedStructures}
          />
          <StatCard
            label="Alliances holding sov"
            value={overview.trackedAlliances}
          />
          <StatCard label="War kills" value={overview.warKills} />
          <StatCard
            label="ISK destroyed"
            value={formatISK(overview.iskDestroyed)}
          />
        </div>
      )}
              ...
              <li key={r.regionId} className="text-sm">
                ...
                <div className="h-1.5 mt-1 overflow-hidden rounded bg-neutral-800">
                  <div
                    ...
                    style={{
                      width: `${(r.campaignCount / maxRegionCount) * 100}%`,
                    }}
                  />
                </div>
              </li>
                        ...
                        <td colSpan={9} className="px-4 py-3">
                          {c.warKills > 0 && (
                            <div className="mb-4">
                              ...
                              <IskWarBar
                                defenderLost={c.defenderIskLost}
                                attackerLost={c.attackerIskLost}
                              />
                            </div>
                          )}
                            ...
                            <>
                              ...
                              <ul className="mt-2 space-y-1">
                                {c.participants.slice(0, 8).map((p) => (
                                  <li
                                    key={p.allianceId}
                                    className="flex items-center justify-between max-w-md text-sm"
                                  >
                                    <AllianceLink
                                      id={p.allianceId}
                                      name={p.allianceName}
...
export default function SovereigntyPage() {
  return (
    <Suspense
      fallback={<Loader fullHeight size="lg" text="Loading sovereignty..." />}
    >
      <SovereigntyContent />
    </Suspense>
  );
